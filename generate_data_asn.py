...
## updated the script to include ASN in the log just for analysis
...
def draw_ip():
    ...
    return (asn, draw_ip_from_asn(asn_list[asn]))
...
def generate_user_login_events(queue, user_id, max_num_of_events):
    ...
    for i in range(num_events):
        (cur_ip, cur_asn) = draw_user_ip(home, work, p_travel, p_home)
        event_log = (user_id, cur_ip, cur_asn)
...
def format_event_as_log(event):
    ...
    #print(event)
    user_id, ip_address, asn = event
    user_name = "user_{}".format(user_id)
    log_line = LOG_FORMAT.format(asn, ip_address, user_name, timestamp_str)
    return log_line
